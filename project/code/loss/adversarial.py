...
def discriminator_loss(discriminator: nn.Module, fake: torch.Tensor, real: torch.Tensor):
    ...
    loss_d = torch.sum(torch.add(-d_fake, d_real))
...
def cycle_loss_sr(generator_a: nn.Module, generator_b: nn.Module, sr: nn.Module, real: torch.Tensor):
    # loss of 2 generators
    temp = generator_a(real)
    temp2 = sr(temp)
    fake = generator_b(temp2)
    
    loss_cycle_sr = F.mse_loss(fake, real)
    return loss_cycle_sr
